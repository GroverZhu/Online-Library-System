...
public class BorrowCartDAO {
    ...
    /**
     * 实现将书添加进购预约车的功能，此时还未提交，因此submit time为null
     * 返回值1表示添加成功
     *
     * @author Liu Zhuocheng
     */
    ...
    public int addBorrowCart(int bookID, int readerID) {
        ...
        if (book == null) {
            ...
            try {
                conn = DatabaseUtil.getInstance().getConnection();
                st = conn.createStatement();
                String sql = "insert into borrow_cart(book_id,reader_id) values(" + bookID + "," +
                        readerID + ")";
                st.executeUpdate(sql);
                st.close();
                conn.close();
        ...
        if (!isexist) {//若不存在已有记录的话
            ...
            if (book != null) {
                if (book.getState().equals("reserve")) {
                    ...
                    String sql = "update borrow_cart set submit_time = \"" + date + "\"" +
    ...
    public int deleteBorrowCart(int bookID, int readerID) {
        ...
        if (!isexist) {//若不存在已有记录的话
            ...
            String sql = "delete from borrow_cart where book_id = " + bookID +
